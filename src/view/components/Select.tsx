    ...
    <div className="relative">
      <label
        className={useClassNames(
          ...
          value && 'text-xs left-[13px] top-2 transition-all translate-y-0'
        )}
      >
        {placeholder}
      </label>
      <RdxSelect.Root onValueChange={handleSelect}>
        ...
        <RdxSelect.Portal>
          <RdxSelect.Content className="z-[99] overflow-hidden bg-white rounded-2xl border-gray-100 shadow-[opx_11px_20px_0px_rgba(0,0,0,0.10)]">
            ...
            <RdxSelect.Viewport className="p-2">
              {options.map((option, idx) =>  <RdxSelect.Item
                key={`option-${idx}`}
                value={option.value}
